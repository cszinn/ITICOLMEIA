...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // 1. Criar a Colmeia com status offline
      const hiveRes = await fetch(`${SUPABASE_URL}/rest/v1/hives`, {
        ...
        body: JSON.stringify({
          ...
          status: 'offline', // Certifique-se de que a constraint no DB permite 'offline'
          temperature: 0,
          humidity: 0,
          total_weight: 0
        })
      ...
      if (!hiveRes.ok) {
        const err = await hiveRes.json();
        // Se ainda falhar, retornamos o erro detalhado do banco para o usuário
        return new Response(JSON.stringify({ 
            error: `Erro no banco de dados: ${err.message || 'Violação de restrição'}. Verifique se o status 'offline' é permitido.` 
        }), { status: hiveRes.status, headers });
      }

      // 2. Inicializar os Favos (default 5)
      ...
      const framesRes = await fetch(`${SUPABASE_URL}/rest/v1/frames`, {
        method: 'POST',
        headers: sbHeaders,
      ...
      if (!framesRes.ok) {
          console.error("Erro ao inicializar favos:", await framesRes.text());
      }
